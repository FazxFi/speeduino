...
void readTPS(bool useFilter)
{
  ...
  if(configPage6.iacAlgorithm == IAC_ALGORITHM_HB)
  {
    ...
    if(configPage9.itpsMax > configPage9.itpsMin)
    {
      //Check that the ADC values fall within the min and max ranges (Should always be the case, but noise can cause these to fluctuate outside the defined range).
      if (currentStatus.itpsADC < configPage9.itpsMin) { tempADC = configPage9.itpsMin; }
      else if(currentStatus.itpsADC > configPage9.itpsMax) { tempADC = configPage9.itpsMax; }
      currentStatus.ITPS = map(tempADC, configPage9.itpsMin, configPage9.itpsMax, 0, 100); //Take the raw ITPS ADC value and convert it into a ITPS% based on the calibrated values
    }
    else
    {
      ...
      //All checks below are reversed from the standard case above
      if (tempADC > configPage9.itpsMin) { tempADC = configPage9.itpsMin; }
      else if(tempADC < configPage9.itpsMax) { tempADC = configPage9.itpsMax; }
      currentStatus.ITPS = map(tempADC, configPage9.itpsMax, configPage9.itpsMin, 0, 100);
    }
  }
  TPS_time = micros();
}
